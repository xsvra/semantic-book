...
export default function BookDetailModal({ book: initialBook, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-6 overflow-y-auto">
      ...
      <div className="relative bg-white border border-border rounded-2xl sm:rounded-3xl shadow-2xl max-w-4xl w-full max-h-[92vh] flex flex-col z-10 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
        {/* Modal Header bar */}
        <div className="px-4 sm:px-6 py-3.5 sm:py-4 border-b border-border flex items-center justify-between bg-slate-900 text-white shrink-0">
          <div className="flex items-center gap-2">
            <BookOpen className="w-4 h-4 sm:w-5 sm:h-5 text-amber-400" />
            <span className="font-serif font-bold text-xs sm:text-sm">Detail Lengkap Buku Non-Fiksi</span>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white flex items-center justify-center transition-colors"
        ...
        <div className="p-4 sm:p-8 overflow-y-auto space-y-5 sm:space-y-6 flex-1 bg-bg-base">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-8">
            {/* Left Column: 3D Cover Image & Metadata Pills */}
            <div className="flex flex-col items-center sm:items-start">
              <div className="w-36 sm:w-full aspect-[2/3] rounded-2xl overflow-hidden bg-slate-900 p-4 relative shadow-md border border-slate-800 mb-4 flex items-center justify-center">
                {book.cover_url && (
                  <div
                ...
                {book.cover_url ? (
                  <img
                    src={book.cover_url}
                    alt={book.title}
                    className="w-24 sm:w-36 aspect-[2/3] object-cover rounded-r-md rounded-l-sm shadow-2xl border-l-4 border-slate-400 border-y border-r border-white/80 z-10 relative"
                  />
                ) : (
              ...
              {book.similarity_score !== undefined && (
                <div className="w-full bg-amber-500/10 text-amber-700 p-2.5 sm:p-3 rounded-2xl text-center border border-amber-400/30 mb-2.5 sm:mb-3">
                  <span className="text-[10px] sm:text-[11px] font-bold block uppercase tracking-wider">Relevansi Cosine Similarity</span>
              ...
              {/* Language Code Badge */}
              <div className="w-full bg-slate-100 px-3 py-2 rounded-xl text-center text-xs font-medium text-slate-700 border border-slate-200 mb-3 sm:mb-4 flex items-center justify-center gap-1.5">
            ...
            <div className="sm:col-span-2 space-y-4 sm:space-y-5">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-1.5 mb-2.5">
                  ...
                  {book.categories && book.categories.map((cat, idx) => (
                    <span key={idx} className="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-amber-500/10 text-amber-800 border border-amber-400/30 text-[11px] sm:text-xs font-bold flex items-center gap-1">
                      <Bookmark className="w-3 h-3 text-amber-600" /> {cat}
                    </span>
                  ))}
                </div>

                <h2 className="font-serif font-bold text-lg sm:text-2xl text-text-primary leading-tight mb-1">
                  {book.title}
                </h2>
                ...
                <div className="p-3 sm:p-4 rounded-2xl bg-white border border-border text-xs space-y-2 shadow-sm">
                  <span className="font-bold text-text-primary flex items-center gap-1">
                    <Tag className="w-3.5 h-3.5 text-amber-600" /> Keseluruhan Genre:
                  </span>
                  <div className="flex flex-wrap gap-1.5 pt-0.5">
                    {allGenresList.map((g, idx) => (
                      <span key={idx} className="px-2.5 py-1 rounded-full bg-slate-100 text-slate-800 text-[11px] sm:text-xs font-medium border border-slate-200">
                        {g}
                      </span>
                    ))}
                  </div>
              ...
              {/* Enhanced Quick Stats Grid — Halaman dan Format DIPISAH (5 Cards Responsive) */}
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 p-3 sm:p-4 rounded-2xl bg-white border border-border text-center shadow-sm">
                ...
                <div className="col-span-2 sm:col-span-1 p-1.5 sm:p-0 border-t sm:border-t-0 border-slate-100 pt-2 sm:pt-0">
              ...
              <div>
                <h4 className="font-serif font-bold text-text-primary text-xs sm:text-sm mb-1.5 sm:mb-2">
                  Deskripsi & Sinopsis Lengkap Buku
                </h4>
                <div className="p-4 sm:p-6 rounded-2xl bg-white border border-border text-xs sm:text-sm text-slate-700 leading-relaxed whitespace-pre-line break-words overflow-hidden shadow-sm">
                  {descriptionText}
                </div>
              </div>
              ...
              {(book.isbn13 || book.isbn10) && (
                <div className="pt-3 border-t border-border text-[11px] sm:text-xs text-text-muted flex flex-wrap gap-3 sm:gap-4">
