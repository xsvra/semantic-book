...
export default function TopRatedGrid({ onSelectBook }) {
  ...
  return (
    <section className="py-12 bg-bg-base border-t border-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 pb-4 border-b border-border/80 gap-4">
          <div>
          ...
          {!isLoading && books.length > 0 && (
            <div className="flex items-center gap-2 bg-white px-3.5 py-1.5 rounded-full border border-border text-xs text-text-secondary shadow-sm shrink-0">
              <TrendingUp className="w-3.5 h-3.5 text-gold" />
              <span>Menampilkan Peringkat #{startIndex + 1} - #{Math.min(startIndex + itemsPerPage, books.length)} dari {books.length} Buku</span>
            </div>
          )}
        </div>
            ...
            {totalPages > 1 && (
              <div className="mt-10 flex items-center justify-center gap-2">
                <button
                  ...
                  className="p-2.5 rounded-xl border border-border bg-white text-text-secondary hover:text-text-primary hover:bg-bg-muted disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm flex items-center gap-1 text-xs font-semibold"
                >
